...
import { listOrders, updateOrderStatus } from "../../api/frontApis";
...
const CheckOrders = () => {
  ...
  const handlePaymentConfirmation = async (orderId: string) => {
    try {
      ...
      const { data } = await updateOrderStatus(orderId, "processing");
      if (data.success) {
        setToast({
          message: "Payment confirmed successfully!",
          type: "success",
        });
        await handleOrdersList();
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-[#6B7280]">
              Confirm payments for customer orders
            </p>
          </div>
          <div className="flex items-center gap-2 text-sm text-[#6B7280] bg-[#FFFBF5] px-4 py-2 rounded-xl border border-[#E5E7EB]">
            ...
            {orders.filter((o) => o.paymentStatus === "unpaid").length} pending
            payments
          </div>
        </div>
                ...
                <div className="space-y-4">
                  ...
                  {selectedOrder.paymentStatus === "unpaid" && (
                    <div className="flex gap-3 pt-2">
                      <button
                        onClick={() =>
                          handlePaymentConfirmation(selectedOrder.id)
                        }
                        disabled={isProcessing}
                        className={`
                        flex-1 flex items-center justify-center gap-2 px-6 py-3 
                        bg-[#D97706] text-white rounded-xl font-medium
                        transition-all duration-200 active:scale-95
                        ${
                          isProcessing
                            ? "opacity-70 cursor-not-allowed"
                            : "hover:bg-[#78350F] shadow-[0_4px_12px_rgba(217,119,6,0.3)]"
                        }
                      `}
                      >
                        {isProcessing ? (
                      ...
                      <button
                        ...
                        className="px-6 py-3 border border-[#E5E7EB] text-[#6B7280] rounded-xl font-medium hover:bg-[#FFFBF5] transition-all duration-200 flex items-center gap-2"
                      >
                        <Printer size={20} />
                        Print
                      </button>
